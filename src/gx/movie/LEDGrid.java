...
public class LEDGrid extends JPanel
{
	...
	public void setPoint(int x, int y, byte color, int frame)
	{
		if(x>=width || y>=height || x<0 || y<0) return;
		byte [] pixels = pixelData.get(frame);
		pixels[y*width+x] = color;		
	}
	
	public void setPoint(int x, int y, byte color) 
	{
		setPoint(x,y,color,currentScreen);
	}
	...
	public byte getPoint(int x, int y, int frame)
	{
	...
	{
		fill(color,currentScreen);
	}
	
	public void fill(byte color, int frame)
	{
		byte [] pixels = pixelData.get(frame);
		for(int x=0;x<pixels.length;++x) pixels[x] = color;		
	}
