...
{	
	...
	class LEDSequenceAction implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			if(t.startsWith("Render")) {
				String className = renderClass.getText().trim();
				try {
					Class<?> c = getClass().getClassLoader().loadClass(className);
					FrameRenderer fr = (FrameRenderer)c.newInstance();
					fr.render(grid);
					grid.updateUI();
				} catch (Exception err) {
					err.printStackTrace();
	...
	public LEDSequence(LEDGrid grid, String filename) throws IOException
	{
		...
		JPanel pRender = new JPanel(new FlowLayout());
		pRender.add(new JLabel("Render Program:  "));
		renderClass = new JTextField("gx.movie.Five",30);
		pRender.add(renderClass);
		JButton btRender = new JButton("Render");
		btRender.addActionListener(action);
		pRender.add(btRender);
		
		JPanel jaa = new JPanel(new BorderLayout());
		jaa.add(BorderLayout.NORTH,ja);
		jaa.add(BorderLayout.SOUTH,pRender);
				
		JPanel jc = new JPanel(new BorderLayout());
		jc.add(BorderLayout.NORTH,jaa);
